...
module.exports = function (params) {
    ...
    app.post('/createtenant', async (req, res, next) => {
        if (req.body && req.body.tenantId && req.body.fullName && req.body.email && req.body.siteName && req.body.loginUrl) {
            if (req.body._id) {
                let updateTenant=await tenantService.updateTenant(req.body)
                if(updateTenant.success){
                    app.http.customResponse(res,updateTenant, 200);
                }else{
                    app.http.customResponse(res, updateTenant, 200);
                }
            }
